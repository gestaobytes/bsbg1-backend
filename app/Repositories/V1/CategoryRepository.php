...
class CategoryRepository implements CategoryInterface
{
    ...
    public function index()
    {
        ...
        if (isset($keywords) && $keywords != '') {
            ...
            $search = "";
            for ($i = 0; $i < $qtd; $i++) {
                $search .= "(title like '%$keywords[$i]%') or (description like '%$keywords[$i]%') or ";
            }
    ...
    {
        return $this->model->select('id', 'title')->orderBy('title')->get();
    }
    ...
    //         ->where('id', $id)
    //         ->select('id', 'name')
    //         ->with('subcategories:id,category_id,name,order,description')
    //         ->get();
    // }
}
