...
use App\Models\Columner;
use App\Interfaces\v1\ColumnerInterface;
use Illuminate\Http\Request;

class ColumnerRepository implements ColumnerInterface
{
    private $model, $request;

    public function __construct(Columner $model, Request $request)
    {
        $this->model = $model;
    ...
    public function index()
    {
        ...
        $pgLimit = isset($_GET['limit']) ? $_GET['limit'] : 2;
        $sortField = isset($_GET['sort']) ? $_GET['sort'] : '-columners.id,users.name,blogs.title';
        $fieldsToSelect = isset($_GET['fields']) ? $_GET['fields'] : 'columners.id,users.name as user,blogs.title as blog';
        ...
        $data = $this->model->select($fieldsToSelect)
            ->leftJoin('users', 'users.id', 'columners.user_id')
            ->leftJoin('blogs', 'blogs.id', 'columners.blog_id')
            ->whereBetween('columners.created_at', [$dts, $dtf]);
        ...
        if (isset($keywords) && $keywords != '') {
            ...
            $search = "";
            for ($i = 0; $i < $qtd; $i++) {
                $search .= "(columners.description like '%$keywords[$i]%' or blogs.title like '%$keywords[$i]%' or users.name like '%$keywords[$i]%') or ";
            }
