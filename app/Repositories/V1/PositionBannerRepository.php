...
use App\Interfaces\v1\PositionBannerInterface;
use Illuminate\Http\Request;

class PositionBannerRepository implements PositionBannerInterface {
    private $model, $request;

    public function __construct(PositionBanner $model, Request $request) {
        $this->model = $model;
    ...
    public function index()
    {
        ...
        $sortField = isset($_GET['sort']) ? $_GET['sort'] : '-id,name';
        $fieldsToSelect = isset($_GET['fields']) ? $_GET['fields'] : 'id,name,description';
        ...
        if (isset($keywords) && $keywords != '') {
            ...
            $search = "";
            for ($i = 0; $i < $qtd; $i++) {
                $search .= "(name like '%$keywords[$i]%') or ";
            }
